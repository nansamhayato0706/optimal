...
final class TrainingService
{
    ...
    public function handleEvent(int $eventType, array $post, array $files): array
    {
        ...
        if ($eventType < 5) {
            $this->imageStorage->store($files, $userUuid, $eventType);
            $contactInserted = $this->trainingRepository->insertContact([
                'contact_uuid' => Uuid::v4(),
        ...
        $this->insertSend($userUuid, $eventType, $payload->hookDiv());
        return $result;
    }
    ...
    private function insertSend(string $userUuid, int $eventType, int $hookDiv): void
    {
        $this->trainingRepository->insertSend([
            'send_uuid'   => Uuid::v4(),
            'send_div'    => $eventType,
