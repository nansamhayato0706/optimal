...
<div class="rf-card"><div class="rf-title">疑問や質問</div><div class="rf-body-split">
	<div class="rf-split-col"><label class="rf-label" for="<?= $h($fieldId('remark')) ?>"><?= $h($fieldLabel('remark')) ?></label><span class="rf-help" id="<?= $h($helpId('remark')) ?>">困っていること、聞きたいこと、体調のこと、嬉しかったことなど、何でも自由に書いてください。255文字以内、5行以内でお願いします。</span><textarea id="<?= $h($fieldId('remark')) ?>" rows="5" name="remark"<?= $readonly ? ' readonly' : '' ?><?= ($errors['remark'] ?? '') !== '' ? ' aria-invalid="true"' : '' ?><?= $describedBy('remark', true) !== '' ? ' aria-describedby="' . $h($describedBy('remark', true)) . '"' : '' ?>><?= $h($form['remark']) ?></textarea><?php if (($errors['remark'] ?? '') !== ''): ?><p class="err" id="<?= $h($errorId('remark')) ?>" role="alert"><?= $h($errors['remark']) ?></p><?php endif; ?><p class="err" id="report_line_limit_message" aria-live="polite"></p></div>
	<div class="rf-split-col"><label class="rf-label" for="<?= $h($fieldId('reply')) ?>"><?= $h($fieldLabel('reply')) ?></label><span class="rf-help" id="<?= $h($helpId('reply')) ?>">255文字以内、5行以内で入力してください。</span><textarea id="<?= $h($fieldId('reply')) ?>" rows="5" name="reply"<?= ($commentOnly && !$locked) ? '' : ' readonly' ?><?= ($errors['reply'] ?? '') !== '' ? ' aria-invalid="true"' : '' ?><?= $describedBy('reply', true) !== '' ? ' aria-describedby="' . $h($describedBy('reply', true)) . '"' : '' ?>><?= $h($form['reply']) ?></textarea><?php if (($errors['reply'] ?? '') !== ''): ?><p class="err" id="<?= $h($errorId('reply')) ?>" role="alert"><?= $h($errors['reply']) ?></p><?php endif; ?></div>
